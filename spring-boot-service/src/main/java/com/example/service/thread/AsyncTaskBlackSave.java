...
@Service
//线程执行任务类
public class AsyncTaskBlackSave {
	Random random = new Random();// 默认构造方法
    @Async
    // 表明是异步方法
    // 无返回值
    public void executeAsyncTask(List<CustomerBlackDO>  list, CustomerBlackDOMapper customerBlackDOMapper) {
    	 System.out.println(Thread.currentThread().getName()+"开启新线程执行" + list.size());
    	 customerBlackDOMapper.batchSave(list);
    }
