...
public class UserDOController {
    ...
    public ResultBean update(@RequestBody @Validated(UpdateGroup.class) UserDO userDO) throws Exception {
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/user/{id}/part", method = RequestMethod.PUT)
    public ResultBean updatePart(@RequestBody @Validated(PartGroup.class) UserDO userDO) throws Exception {
        userDOService.updateFieldById(new UserDO().setId(userDO.getId()).setPassword(userDO.getPassword()).setStatus(userDO.getStatus()));
        return new ResultBean().success();
    }
